...
open class ModelNode : Node {
    ...
    constructor(
        ...
        iTap: ITap? = null
    ) : super(position, rotation, scale, iTap) {
        tap = iTap
    }
     ...
     * TODO : Doc
     */
    constructor(modelInstance: RenderableInstance?) : this(iTap = null) {
    ...
    suspend fun loadModel(
        ...
        centerOrigin: Position? = null,
        render: ModelRenderable? = null,
        onError: ((error: Exception) -> Unit)? = null
    ): RenderableInstance? {
        return try {
            var model: ModelRenderable?
            if (render != null) {
                model = GLBLoader.loadModel1(context, lifecycle, render)
            } else {
                model = GLBLoader.loadModel(context, lifecycle, glbFileLocation)
            }
    ...
    fun loadModelAsync(
        ...
        if (lifecycle != null) {
            ...
            doOnAttachedToScene { sceneView ->
                loadModelAsync(
                    context, sceneView.lifecycle, glbFileLocation, render, autoAnimate, scaleToUnits,
    ...
    override fun clone() = copy(ModelNode(iTap = tap))

    open fun copy(toNode: ModelNode = ModelNode(iTap = tap)): ModelNode = toNode.apply {
        super.copy(toNode)
